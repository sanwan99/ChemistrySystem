...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public void createUser(User user) {
        ...
        //保存UC
        List<Long> list=userChemistryService.getAllChemistryId();
        System.out.println(list);
        for (int i = 0; i <list.size() ; i++) {
            long chemistryId=(long)list.get(i);
            UserChemistry userChemistry =new UserChemistry();
            userChemistry.setStatus(Chemistry.STATUS_VALID);
            userChemistry.setChemistryId(chemistryId);
            userChemistry.setUserId(user.getUserId());
